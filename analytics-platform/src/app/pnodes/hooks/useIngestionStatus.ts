...
const API_POLL_INTERVAL_SECONDS = 30;
...
export function useIngestionStatus({ selectedSeedId }: UseIngestionStatusProps) {
  ...
  useEffect(() => {
    fetchStatus();
    const interval = setInterval(fetchStatus, API_POLL_INTERVAL_SECONDS * 1000);
    return () => clearInterval(interval);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedSeedId]);

  // Update countdown every second based on last API call time
  useEffect(() => {
