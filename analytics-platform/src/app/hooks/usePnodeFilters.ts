...
export function usePnodeFilters({
  ...
  const processedPnodes = useMemo(() => {
    ...
    // Filter by search text (IP address or pubkey)
    if (searchText.trim()) {
      const searchLower = searchText.toLowerCase().trim();
      items = items.filter((n) => {
        const pubkeyMatch = n.pubkey?.toLowerCase().includes(searchLower);
        const addressMatch = n.latestAddress
          ?.toLowerCase()
    ...
    items.sort((a, b) => {
      ...
      // Treat null/undefined uptime OR private nodes as -Infinity for sorting (N/A nodes sort last)
      // This matches the UI behavior where private nodes show N/A
      // This distinguishes N/A from actual 0 uptime (which is a valid value)
      const uptimeA =
        a.isPublic && latestA?.uptimeSeconds != null
          ? latestA.uptimeSeconds
          : -Infinity;
      const uptimeB =
        b.isPublic && latestB?.uptimeSeconds != null
    ...
    return items;
  }, [globalPnodes, reachFilter, sortOption, selectedSeedId, searchText]);
